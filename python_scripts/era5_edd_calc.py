...
# from esgf_access import load_cmip6_year
...
def calculate_daily_edd(ds_tasmax, ds_tasmin):
    ...
    eps = 1e-6  # guard against flat (or nearly-flat) days

    for t in cutoff_temps:
        # ---- SAFE arccos path (avoid divide-by-zero when W == 0) ----
        ...
        # Handle flat days safely (and overrides any NaNs produced above)
        edd = xr.where(np.abs(W) < eps, xr.where(M > t, M - t, 0.0), edd)
    ...
    edd_ds.attrs["source"] = "tasmax/tasmin inputs (Celsius)"
...
def calculate_monthly_edd(edd_ds):
    return edd_ds.resample(time="1M").mean()
